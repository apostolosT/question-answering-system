...
class StackedBiRNN(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.num_layers):
            ...
            rnn_input = outputs[-1]
            rnn_out = F.dropout(rnn_input, p=self.dropout)
            rnn_out, (hidden, cell) = self.rnns[i](rnn_input)
...
class QuAModel(nn.Module):
    ...
    def forward(self, context, question, context_mask, question_mask):
        ...
        ctx_outputs = self.context_birnn(ctx_birnn_input)

        qtn_outputs = self.question_birnn(ques_embed)
